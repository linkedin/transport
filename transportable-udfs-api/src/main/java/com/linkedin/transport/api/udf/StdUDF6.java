...
public abstract class StdUDF6<I1 extends StdData, I2 extends StdData, I3 extends StdData, I4 extends StdData,
   ...
   *
   * The arguments passed to {@link #eval(StdData, StdData, StdData, StdData, StdData, StdData)} are passed to this
   ...
   * depend on columns. For example, in the query {@code SELECT my_udf('my_value', T.Col1) FROM T}, {@literal my_value}
   * is a constant argument to the UDF and its value is the same for all invocations of this UDF, while {@code T.Col1}
   * is a non-constant argument since it is an expression that depends on a table column, and hence its value changes on
   * a per-row basis).
   *
