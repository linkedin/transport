...
public class HiveTester implements SqlStdTester {
  ...
  /**
   * Consumes any exceptions generated by the Hive engine to reveal the underlying exception thrown by the UDF
   */
  private RuntimeException fetchUnderlyingException(Throwable e) {
    while (true) {
      if (e instanceof HiveSQLException || e instanceof HiveException) {
        e = e.getCause();
      } else if (e instanceof IOException && e.getCause() instanceof HiveException) {
        e = e.getCause();
      } else {
        break;
      }
